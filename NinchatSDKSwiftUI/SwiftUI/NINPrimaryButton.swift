...
struct NINPrimaryButtonView: View {
    ...
    private struct DefaultStyle: ButtonOverride {
        var foregroundColor: (Color?, UIColor?)? = (.white, .white)
        var backgroundColor: (Color?, UIColor?)? = (Color(UIColor.defaultBackgroundButton), .defaultBackgroundButton)
        var cornerRadius: CornerRadius? = .rounded
        var borderColor: (Color?, UIColor?)? = nil
    ...
    var body: some View {
        ...
        case .curved(let rad):
            return button.round(radius: rad, borderWidth: override.borderWidth ?? 0.0, borderColor: override.borderColor?.0 ?? .clear)
        case .rounded:
            return button.round(radius: size.height/2, borderWidth: override.borderWidth ?? 0.0, borderColor: override.borderColor?.0 ?? .clear)
        default:
