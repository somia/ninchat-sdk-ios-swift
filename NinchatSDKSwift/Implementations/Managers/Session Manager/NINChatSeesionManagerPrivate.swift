...
extension NINChatSessionManagerImpl {
    ...
    internal func didUpdateQueue(type: Events, param: NINLowLevelClientProps) throws {
        guard let queue = self.queues.first(where: { $0.queueID == param.queueID() }) else { return }
        ...
        do {
            ...
            self.queueUpdateBoundClosures.values.forEach({ $0(type, queue, error) })
        }
    }
    ...
    internal func didUpdateChannel(param: NINLowLevelClientProps) throws {
        ...
        self.channelClosed = channelClosed || channelSuspended
